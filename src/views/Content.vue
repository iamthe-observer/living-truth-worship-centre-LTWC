...
function runParallax(
  ...
  const cardParallax = computed<CSSProperties>(() => ({
    transform: `rotateX(${roll.value * 30}deg) rotateY(${tilt.value * 30
      }deg) scale(110%)`,
...
<template>
  ...
  <div ref="content_container" class="w-full h-full relative flex flex-col bg-transparent z-10">
    <HomeVideo />
    <!-- scroll quick alerts and news -->
    <div class="wrapper border-y-4 border-white shadow-2xl bg-prime text-white font-bold grid place-items-center py-2">
      <div class="marquee">
      ...
      <div v-if="!if_sm" class="grid grid-cols-3 grid-rows-2 gap-2 px-10">
        <div v-for="(source, i) in src" :class="source
          ...
          <div v-if="i == 3" class="uppercase flex justify-between items-end h-full group">
            ...
            <RouterLink to="/visit" class="group-hover:text-3xl transition-all duration-300 ease-out">
              <button
    ...
    <section class="flex flex-col text-[1.5em] py-12 min-h-screen overflow-x-hidden justify-center bg-base100">
      <header v-motion-fade
        ...
        <svg fill="#000000" width="80px" height="80px" viewBox="-2.4 -2.4 28.80 28.80" id="right-arrow"
          ...
          <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
          <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round" stroke="#000"
      ...
      <div class="flex w-full items-center justify-end gap-2 relative pr-10 mt-1">
        <img class="w-6 aspect-square drag_img" src="../assets/icons/drag-left-svgrepo-com.svg" alt="" />
        <span class="drag_txt font-Unbound font-lighter text-sm">Drag to Scroll</span>
        <img class="drag_img w-6 aspect-square" src="../assets/icons/drag-right-svgrepo-com.svg" alt="" />
      </div>
    </section>
