  ...
  <div v-dragscroll.x="true"
    ...
    <div v-motion-fade v-for="(cont, ii) in content" class="" :key="ii">
      <RouterLink
        ...
        <div @click="cont.func ? cont.func() : null" class="flex imtes-center gap-4">
          <span class="text-[1rem] font-Monument group-hover:text-base900 text-prime">{{ cont.link_text }}</span><i
            v-if="cont.link_text" class="pi pi-arrow-right group-hover:text-base900 text-prime text-[1rem]"></i>
        </div>
      </RouterLink>
    </div>
  </div>
